...
def _normalize_model_name(model: str) -> str:
    """Return the short model id (strip optional ``provider/`` prefixes)."""
    return model.strip().split("/")[-1]


def _is_reasoning_model(model: str) -> bool:
    """Return True if the model rejects ``temperature`` / sampling (e.g. o-series)."""
...
class LlmClient:
    ...
    def __init__(
        ...
        self.model = model or os.environ.get("OPENAI_MODEL", "o3-mini")
        env_effort = (os.environ.get("OPENAI_REASONING_EFFORT") or "").strip()
        self.reasoning_effort = reasoning_effort or (env_effort or None)
